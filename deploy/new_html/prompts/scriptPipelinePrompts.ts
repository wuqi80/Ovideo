...
4. **15秒硬上限复核**。任何段落估算超过15秒都是错误，必须继续按情绪锚点、台词停顿或场景/动作转折拆细；不得把超过15秒的内容压缩标成15秒。
...
### 步骤4：最终检查
- 每个段落时长是否为4-15之间的整数？
- 是否存在任何段落自然表演会超过15秒？如果有，必须继续拆分后再输出。
- 是否所有原文都被保留且无修改？
- 平均时长是否≥10秒？
...
2. 原脚本中的时长只做参考，严禁照抄。
3. 每组中镜头总时长严禁大于15秒；如果当前内容自然表演超过15秒，必须拆成多个连续分段/组输出，严禁通过压缩动作或把超时内容标成15秒来规避。
...
23. 输出前必须逐分段累加“时长（秒）”；任何分段累计>15秒都必须重新拆分或重新分配镜头，直到所有分段≤15秒。
...
3. 输出分段号必须从1连续递增；镜头号必须跟随最终输出分段重新连续编号。
4. 每个输出分段的镜头累计时长必须与该分段承载内容的自然表演时长一致，且绝对不得超过15秒。
5. 每个输出分段必须独立包含完整的【视觉风格】和【正向稳定约束】。
6. 输出前逐段计算镜头数量、镜头累计时长和编号；发现局部不合格时只修正该段，再输出最终完整脚本。
...
2. 每组累计时长不超过15秒且镜头数不超过5个；任何超过15秒的组都必须继续拆分或重排，不要向用户暴露质检过程。
