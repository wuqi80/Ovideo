...
将剧本按照以下要求，将剧情拆分成适配竖屏AI仿真人剧的分镜脚本；严禁篡改、增删原文核心剧情、人物设定、旁白、人物对话；严格遵循，每个分镜的运镜动作100%在对应分镜的独立时长内完成闭环，运镜起止节点与分镜时间码完全对齐，运镜速度与分镜时长精准适配，无运镜跨分镜溢出、无时间串帧、无时长压缩、无运镜卡顿、无画面跳帧、无速度骤变，单分镜内仅执行一个核心运镜动作，不叠加多类型运镜。
以纯文字形式输出，具体要求如下：
时长要求：
...
优化补充权限边界·仅可在以下范围内调整：1. 原文单句描述无法用1个分镜完整呈现时，可拆分为2-3个连续分镜；2. 原文剧情出现节奏拖沓、情绪断层、叙事逻辑不顺畅时，可补充不超过2个过渡分镜，补充内容必须服务于原文核心剧情；3. 原文分镜表达无法凸显冲突、放大情绪时，可调整景别、运镜、构图方式，强化剧情张力；4. 所有优化调整必须以还原原文核心为前提，严禁偏离原文剧情。
...
你将一次收到第一步拆分后的全部原文分段，必须逐段一一转换，严禁篡改、增删原文核心剧情、人物设定、旁白、人物对话。`,
  user: `${ITERATION_STAGE2_RULES.replace(
    '将下面这一段剧本，拆解整理成适配竖屏（9:16）AI仿真人剧的视频镜头脚本。',
    '将下面第一步拆分后的全部剧本，逐段拆解整理成适配竖屏（9:16）AI仿真人剧的视频镜头脚本。',
  )}
...
额外硬性要求：
1. 正常情况下输入有多少个分段，输出必须有且仅有多少个分段，并保持原顺序；不得把相邻输入段合并或调换。
2. 只有当某个输入分段按自然表演估算会超过15秒时，才允许把该输入分段拆成多个连续输出分段；拆分后仍必须100%覆盖原分段内容。
