...
额外硬性要求：
1. 输入有多少个分段，输出必须有且仅有多少个分段，并保持原顺序；不得把相邻输入段再次拆开、合并或调换。
2. 输出分段号必须从1连续递增，并与输入分段一一对应。
3. 每个输出分段的镜头累计时长必须与该输入分段标注的时长完全一致，且不得超过15秒。
4. 每个输出分段必须独立包含完整的【视觉风格】和【正向稳定约束】。
5. 输出前逐段计算镜头数量、镜头累计时长和编号；发现局部不合格时只修正该段，再输出最终完整脚本。
...
以纯文字形式输出，具体要求如下：
1.	按原脚本画面分割镜头。
2.	镜头号必须沿用输入所属分段，并以“镜头{分段号}-{段内镜头号}”输出。当前输入首个标准镜头号是{canonicalShotNo}，本次全部标准镜头号及固定输出顺序为：{expectedShotNumbers}。
2.1	输入有多少个标准镜头号，就必须输出多少个镜头设计；逐个一一对应，不得合并、拆分、遗漏、重复或调换顺序。
