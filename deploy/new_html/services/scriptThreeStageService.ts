...
function assertValidSplitSegments(segments: ScriptSegment[]): void {
  if (segments.length === 0) throw new Error('第一步未解析出有效剧本分段');
  ...
  if (durations.some(duration => duration === null || duration < 4 || duration > 15)) {
    throw new Error('第一步分段时长必须全部为4-15秒的正整数');
  }
  ...
  if (denseSegmentRatio < 0.3 || averageDuration < 10) {
    throw new Error('第一步分段未满足14-15秒占比≥30%且平均时长≥10秒的硬性要求');
  }
}
...
export function assertValidVideoScript(
  ...
  groups.forEach((group) => {
    if (group.blocks.length > 5) {
      throw new Error(`分段${group.groupNo}包含${group.blocks.length}个镜头，超过每组5个镜头上限`);
    }
    ...
    if (durations.some(duration => duration === null || duration <= 0 || !Number.isInteger(duration))) {
      throw new Error(`分段${group.groupNo}存在缺失或非正整数镜头时长`);
    }
    const totalDuration = (durations as number[]).reduce((total, duration) => total + duration, 0);
    if (totalDuration > 15) {
      throw new Error(`分段${group.groupNo}累计${totalDuration}秒，超过15秒上限`);
    }
    ...
    group.blocks.forEach((block, index) => {
      ...
      if (block.shotNo !== expected) {
        throw new Error(`分段${group.groupNo}镜头编号不连续：应为${expected}，实际为${block.shotNo}`);
      }
    });
  });
  ...
  if (enforceDurationDensity && (denseGroupRatio < 0.3 || averageGroupDuration < 10)) {
    throw new Error('第二步分段未保持14-15秒占比≥30%且平均时长≥10秒的硬性要求');
  }
}
...
async function splitWithValidation(
  ...
  const segments = await aiSplitScriptIntoSegments(model, originalContent, undefined, taskContext);
...
export async function generateEpisodeVideoScript(
  ...
  if (groups.length === 0) throw new Error('第二步未生成可用的分组视频脚本');
...
export async function generateStoryboardDesignForVersion(
  ...
  for (const group of groups) {
    ...
    for (const block of group.blocks) {
      ...
      const extractions = await aiExtractStoryboardPromptFromVideoShot(
        model,
        block.rawBlock,
        canonicalShotNo,
        options.taskContext,
      ...
      if (extractions.length === 0) {
        throw new Error(`${canonicalShotNo}未提取出有效镜头设计`);
      }
      outputTexts.push(JSON.stringify(extractions));
      extractions.forEach((extraction) => {
        if (!extraction.imagePrompt || !extraction.sceneDescription) {
          throw new Error(`${canonicalShotNo}缺少画面描述或分镜生成提示词`);
        }
  ...
  if (items.length === 0) throw new Error('第三步未生成任何镜头设计卡片');
