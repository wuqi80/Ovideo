...
set COMFY_PORT=8188
set COMFY_ROOT=E:\OSTORY-GPU\ComfyUI_windows_portable
set PYTHON_EXE=%COMFY_ROOT%\python_embeded\python.exe
set COMFY_MAIN=%COMFY_ROOT%\ComfyUI\main.py
set LOG_FILE=E:\OSTORY-GPU\logs\comfyui.log
set CLEANUP_SCRIPT=%~dp0windows_gpu_cleanup_port.ps1
if not exist "%COMFY_ROOT%\python_embeded\python.exe" (
  echo [ERROR] missing python embed: E:\OSTORY-GPU\ComfyUI_windows_portable\python_embeded\python.exe
  exit /b 2
)
if not exist "%COMFY_MAIN%" (
  echo [ERROR] missing ComfyUI main.py: E:\OSTORY-GPU\ComfyUI_windows_portable\ComfyUI\main.py
  exit /b 3
)
...
echo [OSTORY] start comfyui %COMFY_PORT% at %date% %time% >> "%LOG_FILE%"
cd /d "%COMFY_ROOT%"
