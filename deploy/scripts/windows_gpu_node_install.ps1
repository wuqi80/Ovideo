...
if (Test-Path $seedRequirements) {
    ...
    & $Python -s -m pip install -r $seedRequirements
    if ($LASTEXITCODE -ne 0) {
...
& $Python -s -m pip install requests pillow
if ($LASTEXITCODE -ne 0) {
...
python_embeded\python.exe -s ComfyUI\main.py --listen 127.0.0.1 --port 8188 --lowvram --preview-method none --disable-auto-launch >> E:\MECHA-GPU\logs\comfyui.log 2>&1
endlocal
'@
