...
function Download-VerifiedFile {
    ...
    & $Curl --location --fail --retry 30 --retry-all-errors --retry-delay 10 --connect-timeout 30 --continue-at - --output $partial $Url
    if ($LASTEXITCODE -ne 0) {
        throw "Model download failed with exit code ${LASTEXITCODE}: $Url"
    }
...
$Models = @(
    @{
        Url = "https://huggingface.co/Kijai/WanVideo_comfy_fp8_scaled/resolve/main/I2V/Wan2_1-I2V-14B-480p_fp8_e4m3fn_scaled_KJ.safetensors?download=true"
    ...
    @{
        Url = "https://huggingface.co/Kijai/WanVideo_comfy_fp8_scaled/resolve/main/InfiniteTalk/Wan2_1-InfiniteTalk-Single_fp8_e4m3fn_scaled_KJ.safetensors?download=true"
...
if (-not $SkipModelDownloads) {
    foreach ($model in $Models) {
        Download-VerifiedFile -Url $model.Url -Destination $model.Destination -Sha256 $model.Sha256
    }
}
