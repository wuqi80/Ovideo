...
def check_cors_allowlist_contract(root: Path) -> int:
    ...
    required_snippets = [
        ...
        (root / "cluster_config.py", '"https://tv.ostory.ai,"'),
        ...
        (root / "auto_deploy_cluster.py", '"https://tv.ostory.ai,"'),
...
def check_project_routes_extracted(root: Path) -> int:
    ...
    required_snippets = [
        ...
        (router_text, "clear_project_video_tasks_response(", projects_path),
        ...
        (video_task_service_text, "version_dao.get_project_versions(project_id)", project_video_task_service_path),
        (video_task_service_text, "persist_export_storyboard_base64_image", project_video_task_service_path),
        (video_task_service_text, "data[\"video_tasks\"] = video_tasks", project_video_task_service_path),
        (video_task_service_text, "await project_dao.save_or_update_project(", project_video_task_service_path),
    ]
    ...
    clear_text = router_text[clear_start:]
    export_forbidden = [
    ...
    for snippet in export_forbidden:
        if snippet in export_text:
            fail(f"routers/projects.py must delegate export-to-video workflow to service: {snippet}")
    for snippet in clear_forbidden:
        if snippet in clear_text:
            fail(f"routers/projects.py must delegate clear-video-tasks workflow to service: {snippet}")
    return (
        route_count
...
def check_storyboard_routes_extracted(root: Path) -> int:
    ...
    required_snippets = [
        ...
        (service_text, "storyboard_dao.replace_batch(", storyboard_service_path),
...
def check_audio_routes_extracted(root: Path) -> int:
    ...
    required_snippets = [
        ...
        (audio_text, "attach_local_generated_audio_file(", audio_path),
        (audio_text, "get_audio_provider('minimax')", audio_path),
        (audio_text, "media_source='generated_audio_minimax_speech'", audio_path),
...
def check_api_provider_runtime_model_contract(root: Path) -> int:
    ...
    for snippet in (
        ...
        if snippet not in ai_proxy_gemini_image_text:
    ...
    for forbidden in (
        ...
        checks += 1
    if "_post_doubao_image_generation(" not in doubao_image_source:
        fail("generate_doubao_images must delegate provider HTTP handling to _post_doubao_image_generation()")
    checks += 1
    for forbidden in (
        "_post_json_request_async(",
        "config.url_for(",
...
def check_frontend_http_client_contract(root: Path) -> int:
    ...
    required_snippets = [
        ...
        (seedance_multimodal_panel, "supportsMultimodal?: boolean;"),
        (seedance_panel_with_candidates, "supportsMultimodal?: boolean;"),
        (video_page, "supportsMultimodal={seedanceSupportsMultimodal(group.model)}"),
...
def check_admin_api_config_ui_contract(root: Path) -> int:
    ...
    required_snippets = [
        ...
        "formatEndpoint(runtimeEndpoint || dbEndpoint)",
    ...
    forbidden_snippets = [
        ...
        "formatEndpoint(config.endpoint || runtime?.endpoint)",
