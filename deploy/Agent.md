...
- Hong Kong to mainland-China providers can remain slow even when connectivity is valid.
- Use the existing provider-level `proxy_mode=custom` / `custom_proxy` or global system proxy to route selected providers through a mainland VPS.
- Do not hard-code proxy behavior in generation handlers.
...
- Added `services/ai_proxy_doubao_image_service.py` for Doubao image payload construction, runtime provider resolution, HTTP generation calls, and response parsing.
- Moved `build_doubao_image_payload()`, `parse_doubao_image_response()`, `_post_doubao_image_generation()`, and `generate_doubao_images()` out of `services/ai_proxy_service.py`.
- Kept the same public Doubao image entrypoints re-exported from `services/ai_proxy_service.py` for existing routers and tests.
...
- Local `py_compile` for Doubao image service, provider service, and route/provider contracts: passed.
- Local targeted image content, reference, and provider runtime pytest: `47/47` passed.
- Local `scripts/check_provider_contract.py`: passed, including `runtime_wired_files=14`.
- Local `scripts/check_route_contract.py`: passed, including `api_provider_runtime_model_checks=439` and `service_mapper_purity_checks=1539`.
- Local `scripts/check_architecture_contracts.py`: passed `10/10`.
- Local `git diff --check`: passed.
- Local smoke test: `9/9` passed.
...
- Local `py_compile` for shared type, provider, router, image content, reference, and provider-runtime test files: passed.
- Local targeted image content, reference, and provider runtime pytest: `47/47` passed.
- Local `scripts/check_route_contract.py`: passed, including `api_provider_runtime_model_checks=307` and `service_mapper_purity_checks=1379`.
- Local `scripts/check_architecture_contracts.py`: passed `10/10`.
- Local `git diff --check`: passed.
- Local smoke test: `9/9` passed.
- Deployed to `https://mecha.one/`; `drama.service` stayed `active`, remote architecture contracts passed `10/10`, and online smoke passed `9/9`.
...
- Added `services/ai_proxy_image_content_service.py` to own generated image content loading from data URLs and provider-hosted public URLs.
- Updated `services/ai_proxy_image_persistence_service.py` to use the new content loader service instead of importing from `ai_proxy_service.py`.
- Removed generated image content decoding/downloading, `base64`, and public URL guard imports from `services/ai_proxy_service.py`.
- Added `tests/test_ai_proxy_image_content_service.py` for data URL decoding and public URL download behavior.
- Strengthened `scripts/check_route_contract.py` so provider service cannot reintroduce generated image content loading.
...
- Local `py_compile` for `services/ai_proxy_service.py`, `tests/test_api_provider_runtime_model_env.py`, and `scripts/check_route_contract.py`: passed.
- Local targeted provider runtime pytest: `41/41` passed.
- Local `scripts/check_route_contract.py`: passed, including `api_provider_runtime_model_checks=274`.
- Local `scripts/check_architecture_contracts.py`: passed `10/10`.
- Local `git diff --check`: passed.
- Local smoke test: `9/9` passed.
- Deployed to `https://mecha.one/`; `drama.service` stayed `active`, remote architecture contracts passed `10/10`, and online smoke passed `9/9`.
...
- Local `py_compile` for `services/ai_proxy_service.py`, `tests/test_api_provider_runtime_model_env.py`, and `scripts/check_route_contract.py`: passed.
- Local targeted provider runtime pytest: `39/39` passed.
- Local `scripts/check_route_contract.py`: passed, including `api_provider_runtime_model_checks=267`.
- Local `scripts/check_architecture_contracts.py`: passed `10/10`.
- Local `git diff --check`: passed.
- Local smoke test: `9/9` passed.
- Deployed to `https://mecha.one/`; `drama.service` stayed `active`, remote architecture contracts passed `10/10`, and online smoke passed `9/9`.
...
- Refactored `services/ai_proxy_service.py` so `generate_gemini_images()` delegates provider HTTP handling to `_post_gemini_image_generation()`.
- Added `parse_gemini_image_response()` to centralize Gemini inline image extraction from `candidates[].content.parts[].inlineData`.
- Strengthened `tests/test_api_provider_runtime_model_env.py` with parser coverage for inline images and empty image responses.
- Strengthened `scripts/check_route_contract.py` so `generate_gemini_images()` cannot reintroduce direct `_post_json_request_async()` calls or inlineData parsing.

### Verification

- Local `py_compile` for `services/ai_proxy_service.py`, `tests/test_api_provider_runtime_model_env.py`, and `scripts/check_route_contract.py`: passed.
- Local targeted provider runtime pytest: `37/37` passed.
- Local `scripts/check_route_contract.py`: passed, including `api_provider_runtime_model_checks=258`.
- Local `scripts/check_architecture_contracts.py`: passed `10/10`.
- Local `git diff --check`: passed.
- Local smoke test: `9/9` passed.
- Deployed to `https://mecha.one/`; `drama.service` stayed `active`, remote architecture contracts passed `10/10`, and online smoke passed `9/9`.
...
- Local `py_compile` for `services/ai_proxy_service.py`, `tests/test_api_provider_runtime_model_env.py`, and `scripts/check_route_contract.py`: passed.
- Local targeted provider runtime pytest: `35/35` passed.
- Local `scripts/check_route_contract.py`: passed, including `api_provider_runtime_model_checks=253`.
- Local `scripts/check_architecture_contracts.py`: passed `10/10`.
- Local `git diff --check`: passed.
- Local smoke test: `9/9` passed.
- Deployed to `https://mecha.one/`; `drama.service` stayed `active`, remote architecture contracts passed `10/10`, and online smoke passed `9/9`.
...
- Local `py_compile` for `services/ai_proxy_service.py`, `tests/test_api_provider_runtime_model_env.py`, and `scripts/check_route_contract.py`: passed.
- Local targeted provider runtime pytest: `35/35` passed.
- Local `scripts/check_route_contract.py`: passed, including `api_provider_runtime_model_checks=250`.
- Local `scripts/check_architecture_contracts.py`: passed `10/10`.
- Local `git diff --check`: passed.
- Local smoke test: `9/9` passed.
- Deployed to `https://mecha.one/`; `drama.service` stayed `active`, remote architecture contracts passed `10/10`, and online smoke passed `9/9`.
...
- Moved `/api/minimax/tts/sync` fast-path workflow from `routers/audio.py` into `services/audio_generation_service.py`.
...
- Extended `tests/test_audio_generation_service.py` to cover successful sync TTS save/media sync/voice binding, empty and overlong text validation, provider failure, missing audio bytes, and nonfatal media/voice write-back failures.
- Strengthened `scripts/check_route_contract.py` so `/api/minimax/tts/sync` cannot regress to route-local provider calls, `audio_bytes` handling, file saves, media-library sync, or character voice URL write-back.
...
- Moved AI proxy task row creation/completion for `/api/deepseek/chat`, `/api/gemini/text`, `/api/gemini/image`, and `/api/materials/doubao` from `routers/ai_proxy.py` into `services/ai_proxy_task_service.py`.
...
- Added `tests/test_ai_proxy_task_service.py` to cover DeepSeek task creation, text result completion/truncation, Gemini text task create+complete, image task create+complete, and nonfatal DAO failures.
- Strengthened `scripts/check_route_contract.py` so AI proxy routes cannot regress to route-local `TaskDAO` imports/calls or `time.time()` task id generation.
- Added the new AI proxy task service test to `scripts/live_deploy_mvc2.sh` so server sync includes it.
...
- Image/storyboard quality consistency, intermittent next-day missing media, and deeper storyboard generation partial failures remain open and need separate data/provider investigation.
...
- Added canonical storage path helper `utils/storage_layout.py`.
...
- Added SQL migration `sql/db_migration_files_project_episode_source.sql` to add and backfill `files.project_id`, `files.episode_id`, and `files.source`.
- Added read-only audit script `scripts/audit_storage_manifest.py` for missing DB refs, disk orphans, and unowned media files.
- Added dry-run restructure manifest script `scripts/restructure_storage_manifest.py`; it generates CSV and SQL update plans but does not move/delete files.
- Added `sql/` plus the two storage migration scripts to `scripts/live_deploy_mvc2.sh` so server deployments do not miss migration tooling.
- Added clean migration package script `scripts/build_clean_migration_package.py` to export only valid referenced files and generate new-server cleanup SQL for excluded dirty records.
...
- Use `scripts/package_storage_orphans.py --archive` after `scripts/audit_storage_manifest.py` has produced `storage_audit_reports/disk_orphans.csv`.
- The orphan package must not be copied to the new production server wholesale; only owner-selected files should be re-uploaded later.
...
- Permanent purge requires `risk_ack=true` and is restricted to files under `persistent_storage`; this keeps local disk cleanup from touching code or unrelated server files.
- Existing admin media deletion already soft-deletes `media_library_items` and related `files`, so the recycle bin can restore those rows or permanently release local disk space.
- Added multi API key management:
  - `POST /api/admin/api-configs/bulk-keys` creates multiple key rows for one provider.
  - `POST /api/admin/api-configs/{config_id}/activate` makes one key active and disables sibling keyed rows for the same provider.
- Updated `new_html/admin/AdminSettingsPage.tsx` with batch-key input, active-key badge, key suffix preview, active-key switch button, and a recycle-bin panel.
...
- Fixed final task failure persistence in `core/task_queue.py`: when Redis reaches the final failed state, SQL `tasks.status` is now updated to `failed` with the error message.
...
- Updated `scripts/check_gpu_agent_readiness.py` so GPU Agent restart commands use `PUBLIC_BASE_URL` / `SERVER_BASE_URL` / `SMOKE_BASE_URL` before falling back to `https://mecha.one`.
...
- Deployment prerequisite: copy the new runner/setup/smoke files to `E:\MECHA-GPU`, run `windows_gpu_wan_setup.cmd` as Administrator, then run `windows_gpu_wan_smoke.cmd i2v` and `windows_gpu_wan_smoke.cmd infinitetalk`. GPU2 completed both real tests on 2026-07-14: I2V produced `MECHA_GPU2_wan_i2v_00001.mp4` in 490.97 seconds and InfiniteTalk produced `MECHA_GPU2_infinitetalk_00001-audio.mp4` in 501.15 seconds.
- If a Windows restart removes or disables the GPU2 startup tasks, run `scripts/windows_gpu_task_repair.cmd` on the node to recreate and launch the ComfyUI and Agent tasks. The tasks run as `SYSTEM`, start before login, and restart failed processes every minute without storing an Administrator password.
...
- 2026-07-14: Real dubbing E2E exposed `seed=-1` being rejected by GPU2's `WanVideoSampler`. Both `prepare_video_voice_task()` and the Windows GPU2 runner now normalize invalid or negative seeds to a non-negative random integer before workflow submission.
- 2026-07-14: GPU1 probes accepted video/audio/T5/VAE/Wav2Vec/CLIP components, but its current Wan/InfiniteTalk model and LoRA inventory no longer matches the legacy repository template. The legacy GPU1 Agent also hides `/prompt` response bodies, so guessed model names were not deployed. Update that Agent and inspect live `object_info` once GPU1 host access is available; GPU2 dubbing remains independently testable.
- 2026-07-14: Video-reverse detail polling now tolerates two transient gateway failures, reports only sustained retry conditions, and clears the polling error after the next successful JSON response so a deployment-time 502 cannot remain stuck in the UI.
- 2026-07-14: Video-reverse detail polling also starts when the initial detail request failed and `selectedTask` is still empty. A selected task ID is now sufficient to retry every three seconds; only three consecutive failures surface the retry notice.
