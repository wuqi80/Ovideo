...
- 2026-07-14：补齐视频反推首次详情请求失败后的自动重试。即使页面尚未取得 `selectedTask`，只要已有任务 ID 就继续每 3 秒拉取详情；连续 3 次失败才提示，避免首次 502 让轮询永远无法启动。
- 2026-07-14：GPU2 配音真实验收任务约 8 分钟完成并回传 MP4。GPU2 当前不提供 ComfyUI 中间百分比，美化页不再显示误导性的 `0%`，改为“GPU 处理中，可能需要数分钟”；仅收到大于 0 的真实进度时才展示百分比。
- 2026-07-14：GPU2 InfiniteTalk 不再把 33 帧低显存窗口误作整段任务总长度。美化页把目标片段时长传入服务端，Agent 按 16 FPS 动态计算总帧数（最长 30 秒），同时继续以 33 帧窗口分段处理，保证 RTX 3060 12G 的显存策略不变。
- 2026-07-14：真实验收所用参考音频约 0.68 秒，因此回传约 0.64 秒带音频视频属于 `trim_to_audio` 的正常结果；较长音频由动态总帧逻辑覆盖。相关 GPU2 runner 与服务测试 25 项通过，前端生产构建通过。
- 2026-07-14：视频结果展示不再把最近一次任务状态当成历史资源状态。后续调用其他模型失败时，已有成功视频继续正常显示和播放，只在标题处提示“本次失败，历史结果已保留”；新增纯函数和回归测试防止再次灰化历史视频。
- 2026-07-15：Yuan 账号 SB-8 的 Seedance 失败不是历史视频覆盖问题，该镜头没有已完成视频；实际失败源于当前参考图的文件记录已进入回收站但物理文件仍存在，worker 解析 `/storage/...?...token` 时因 `is_deleted = TRUE` 查不到文件。`dao/content/content.py` 现在支持去 token、去域名、兼容 image/images 前缀，并允许 worker 对当前任务引用的素材使用 `include_deleted=True` 解析物理文件；前端 DB 兜底只补历史视频，不再把失败状态强制改成完成。
- 2026-07-15：部署契约要求 AI proxy router 将图片任务持久化委托到 service。GPT Image 路由已改为通过 `create_completed_image_task()` 创建并完成任务记录，并把 `source_task_id` 传给图片持久化服务；`check_route_contract.py` 本地通过。
